...
import { UserProfile } from '@/types/quietPlace';
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed right-4 top-16 z-50 w-80 overflow-hidden rounded-3xl border border-white/20 bg-white/95 shadow-2xl backdrop-blur-xl"
          >
            {/* Header with avatar */}
            {/* 头部和头像 */}
            ...
            <div className="relative z-10 -mt-6 grid grid-cols-2 gap-3 px-4">
              <div className="rounded-2xl border border-rose-100 bg-white p-3 shadow-md">
                <div className="flex items-center gap-1.5 text-rose-500">
                  <Heart size={14} />
                  <span className="text-[10px] font-semibold uppercase">Favorites</span>
                  {/* 收藏 */}
                </div>
                <p className="mt-1 text-xl font-bold text-slate-800">{profile.favorites.length}</p>
              </div>
              <div className="rounded-2xl border border-amber-100 bg-white p-3 shadow-md">
                <div className="flex items-center gap-1.5 text-amber-500">
                  <Bookmark size={14} />
                  <span className="text-[10px] font-semibold uppercase">Saved</span>
                  {/* 已保存 */}
                </div>
                <p className="mt-1 text-xl font-bold text-slate-800">{profile.savedPlaces.length}</p>
              </div>
            </div>
            ...
            <div className="max-h-64 overflow-y-auto p-4">
              ...
              {/* Saved places list */}
              {/* 已保存地点列表 */}
              <h3 className="mb-3 mt-4 text-xs font-semibold uppercase tracking-wide text-slate-400">
                Saved Places
                {/* 已保存地点 */}
              </h3>
              <div className="space-y-2">
                {profile.savedPlaces.map((place) => (
                  <div
                    ...
                    className="flex items-center gap-3 rounded-xl bg-slate-50 p-3"
                  >
                    <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-amber-100 text-amber-500">
                      <Bookmark size={14} />
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-xs font-medium text-slate-700">
